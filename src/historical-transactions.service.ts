...
type HistoricalTransactionsRequest = {
  ...
  /** Internal use for daily sync: process every company currently loaded in Tally. */
  syncAllLoadedCompanies?: boolean;
  /**
...
export async function runHistoricalTransactionsSync(
  ...
  try {
    const companies = await getCompaniesForSync({
      ...
      syncAllLoadedCompanies: Boolean(input?.syncAllLoadedCompanies),
    });
