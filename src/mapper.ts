...
function readPartyLedgerAmount(voucherBlock: string) {
  const ledgerBlocks =
    voucherBlock.match(
      /<LEDGERENTRIES\.LIST\b[\s\S]*?<\/LEDGERENTRIES\.LIST>/gi,
    ) || [];

  for (const ledgerBlock of ledgerBlocks) {
    const isPartyLedger =
      stripXml(readTag(ledgerBlock, "ISPARTYLEDGER")).trim().toLowerCase() ===
      "yes";

    if (!isPartyLedger) continue;

    const amount = toPositiveNumber(readTag(ledgerBlock, "AMOUNT"));
...
function extractUnitFromQuantityOrRate(
  quantity?: string | null,
  rate?: string | null,
) {
  const quantityMatch = String(quantity || "").match(/^[-\d.,]+\s+(.+)$/i);

  if (quantityMatch?.[1]) {
    return quantityMatch[1].trim();
  }

  const rateMatch = String(rate || "").match(/\/(.+)$/);
  ...
  return itemBlocks
    .map((itemBlock, index) => {
      ...
      const unit =
        ...
        extractUnitFromQuantityOrRate(billedQtyRaw || actualQtyRaw, rateRaw);
...
function parseVoucherOrders(
  ...
  return voucherBlocks
    .map((block) => {
      ...
      const partyLedgerAmount = readPartyLedgerAmount(block);

      const subtotalAmount = itemsTotal;

      const totalAmount =
        options.voucherNature === "delivery_challan"
          ? partyLedgerAmount || voucherAmount || itemsTotal
          : voucherAmount || itemsTotal;
      ...
      return {
        ...
        placeOfSupply: stripXml(readTag(block, "PLACEOFSUPPLY")) || null,
        place_of_supply: stripXml(readTag(block, "PLACEOFSUPPLY")) || null,

        companyGstState: stripXml(readTag(block, "CMPGSTSTATE")) || null,
        company_gst_state: stripXml(readTag(block, "CMPGSTSTATE")) || null,

        dispatchThrough: stripXml(readTag(block, "BASICSHIPPEDBY")) || null,
        dispatch_through: stripXml(readTag(block, "BASICSHIPPEDBY")) || null,

        destination: stripXml(readTag(block, "BASICFINALDESTINATION")) || null,
      };
    })
    .filter(Boolean);
}
...
export function parseDeliveryChallans(xml: string) {
  return parseVoucherOrders(xml, "Delivery Note", {
    trustSourceCollection: true,
    excludeOrderVoucherTypes: true,
    voucherNature: "delivery_challan",
  });
}
